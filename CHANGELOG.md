...
All notable changes to `@kubit/agent-plugin` are documented here.
...
- `npx @kubit/agent-plugin` installer for Claude Code and Cursor, with `--global` / `--local` scope and a scratch `--config-dir` override.
