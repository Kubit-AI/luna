# OpenTelemetry GenAI Source Adapter (blame)
...
**Detection trap.** `@opentelemetry/api` alone in `package.json` (TS)
without any GenAI instrumentation dep or `gen_ai.*` literal in source
is not a GenAI source — many TS apps pull OTel in for general HTTP
tracing. Skip this adapter when no GenAI marker is present.

## 2. Trace-shape schema

**Primary instrumentation scopes** (any one indicates an OTel GenAI
provider auto-instrumentation; matches the `opentelemetry.instrumentation.*`
prefixes in `KNOWN_LLM_INSTRUMENTATION_SCOPE_PREFIXES`):
...
Hand-rolled OTel emission (calling `tracer.start_span(...)` directly
with `gen_ai.*` attributes) appears under whatever tracer name the
user picked — fall back to attribute-based detection (`gen_ai.*`
present) when scope alone is inconclusive.
