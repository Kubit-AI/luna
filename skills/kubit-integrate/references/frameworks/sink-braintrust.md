...
- A module that calls `setupOtelCompat()` from `@braintrust/otel` and
  ...
  processor to the same `spanProcessors` array. Do **not** wrap it in
  a `BRAINTRUST_OTEL_COMPAT` env-var check — the env var has no
...
- **LangChain caveat.** When `langchain` is in `sources_detected`,
  ...
  Braintrust setup. Merging `KubitSpanProcessor` into the existing
...
- **Path B — OTel-native instrumentation.** Shape differs by
  ...
  - **Python.** `opentelemetry-instrumentation-langchain` plus a
    ...
    `KubitSpanProcessor` (registered in §3 above) receive them.
    Adds the OpenLLMetry instrumentor ecosystem and requires
  ...
  - **TypeScript (parallel pipelines).** Different shape — the §3
    ...
    Gated on the published `@kubit-ai/otel` allowing OpenInference
    scopes through its default span filter (see SKILL.md Gotchas).
...
- When `langchain` is also in `sources_detected` (see §3b), the
  ...
  - **Path B** (OTel-native instrumentation). Shape differs by
    ...
    - TypeScript (parallel pipelines):
      ...
      `source-langchain.md` §6). Gated on the published
      `@kubit-ai/otel` allowing OpenInference scopes through its
