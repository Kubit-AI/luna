...
- **OTel shape** — the standalone bootstrap stands up one
  ...
  Kubit's `BatchSpanProcessor(OTLPTraceExporter(...))` in
  `spanProcessors: [...]`. Both destinations share the same provider;
  Langfuse keeps shipping to Langfuse and Kubit gets its own copy.
...
a `BatchSpanProcessor(OTLPTraceExporter(...))` aimed at the Kubit
ingest endpoint to the user's existing provider. See the
...
```typescript
// Generated by /kubit-integrate for langfuse on <YYYY-MM-DD>.
// Requires KUBIT_API_KEY and KUBIT_OTEL_ENDPOINT env vars plus
...
- A module that imports from `@langfuse/otel` and constructs a
  ...
  processors after construction, so a separate
  `provider.register()` would clobber Langfuse's registration.
...
`LangfuseSpanProcessor` and Kubit's
`BatchSpanProcessor(OTLPTraceExporter(...))` are registered on
...
- TypeScript: `npm install @opentelemetry/api @opentelemetry/exporter-trace-otlp-proto @opentelemetry/resources @opentelemetry/sdk-trace-base @opentelemetry/sdk-trace-node`.
  Match the project's existing OTel SDK major when any of these are
  already declared; otherwise install the latest. The OTel-shape
  standalone form additionally imports from `@langfuse/otel` — it's
  already in `package.json` per §1 (that's how this branch is
  chosen), so no extra install. The native-shape standalone form
  needs no Langfuse-side extras.
...
```bash
KUBIT_API_KEY=<your-key> KUBIT_OTEL_ENDPOINT=<your-endpoint> \
  npx tsx -e "
